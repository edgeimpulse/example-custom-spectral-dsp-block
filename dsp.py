    ...
    def calculate_edges(f, Pxx_den):
        ...
        for k in range(0, len(edges)):
            ...
            for px in range(0, len(Pxx_den)):
                if (f[px] >= edges[k][0] and f[px] < edges[k][1]):
                    sum += Pxx_den[px]
                    total += 1
...
def generate_features(implementation_version, draw_graphs, raw_data, axes, sampling_freq, scale_axes,
                      filter_type, filter_cutoff, filter_order, fft_length):
    ...
    spectral_power_edges = list(map(lambda x: [ x - 0.5, x + 0.5 ],  np.arange(1, sampling_freq / 2)))
    ...
    for k in range(0, len(spectral_power_edges)):
        labels.append('Spectral Power ' + str(spectral_power_edges[k][0]) + ' - ' + str(spectral_power_edges[k][1]))
...
if __name__ == "__main__":
    ...
    try:
        processed = generate_features(1, args.draw_graphs, raw_features, raw_axes, args.frequency, args.scale_axes, args.filter_type, args.filter_cutoff,
            args.filter_order, args.fft_length)
